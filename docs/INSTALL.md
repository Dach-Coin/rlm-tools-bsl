...
#### Запуск как дочерний процесс MCP-клиента

Под stdio сервер запускает не пользователь, а MCP-клиент, и запускает его **своим**
процессом. Наследование окружения при этом ненадежно: клиент может стартовать из
сеанса без вашего профиля, из службы, из контейнера или с урезанным набором
переменных. Поэтому все нужные переменные задавайте **в секции `env` записи
MCP-сервера**, а не рассчитывайте, что они доедут сами.

Минимальный набор, который стоит задать явно:
  ...
  "mcpServers": {
    "rlm-tools-bsl": {
      ...
      "env": {
        ...
        "RLM_CONFIG_FILE": "D:\\rlm-tools-bsl\\service.json",
...
Что важно знать про каждую из них:

- **`RLM_INDEX_DIR` должна быть абсолютной.** Относительный путь считается от
  текущего каталога **процесса сервера**, а его выбирает клиент — не вы.
- **Пустое значение молча игнорируется.** `"RLM_INDEX_DIR": ""` — это не «не
  задана»: ключ формально присутствует, поэтому значение из `.env` его уже не
  перекроет (`load_dotenv` вызывается с `override=False`). Корень тихо уедет по
  СЛЕДУЮЩЕМУ правилу: при заданной `RLM_CONFIG_FILE` — в
  `dirname(RLM_CONFIG_FILE)/index`, а если и её нет — в `~/.cache`. То есть в
  примере выше индексы окажутся в `D:\rlm-tools-bsl\index`, а не там, где вы их
  ждете. С версии 1.35.2 фактический корень и правило видны в стартовой строке
  лога.
- **`RLM_INDEX_DIR` переносит индексы, но НЕ файловый кеш.** Кеш живет рядом с
  `RLM_CONFIG_FILE`, а без него — в `~/.cache/rlm-tools-bsl`. На профиле, где нет
  доступа к `%USERPROFILE%\.cache`, задавайте **обе** переменные.
- **`RLM_CONFIG_FILE` может указывать на еще не существующий `service.json`.**
  Сам файл сервер не создает и без него работает штатно — переменная нужна как
  указатель на КАТАЛОГ, а уже он определяет, где окажутся `projects.json`,
  `logs/` и `cache/`. Файл появляется при `service install`.

Какие каталоги создаются и когда:

| Каталог | Когда создается |
| --- | --- |
| `index/` | при первой сборке индекса |
| `cache/` | при первом хелпере сессии, которому нужен каталог модулей |
| `logs/` | только при HTTP-транспорте; **под stdio НЕ создается** |

**Под stdio `server.log` не ведется вовсе** — весь лог уходит в stderr, и куда
именно попадет этот stderr, решает MCP-клиент. Чтобы проверить, что сервер
подхватил вашу конфигурацию, найдите в его выводе строку вида:

```
startup: version=<версия> transport=stdio sandbox_mode=process strategy_mode=slim RLM_CONFIG_FILE=<путь|not set|(blank)> RLM_INDEX_DIR=<путь|not set|(blank)> index_root=<корень> (<правило>) cache_root=<корень> log=stderr
```

`index_root=` и `cache_root=` — это фактические корни, а в скобках после
`index_root` стоит правило, по которому он выбран (`RLM_INDEX_DIR`,
`RLM_CONFIG_FILE` или `по умолчанию`). Если корень оказался не там, где вы
ожидали, — строка сразу говорит, какая переменная сработала.
...
**`service uninstall` настройки НЕ удаляет** (v1.35.0). Это половина каждого обновления: скрипты установки снимают службу и ставят её заново, и раньше на этом шаге пропадали адрес, порт и путь к `.env` — служба возвращалась на `127.0.0.1:9000` (issue [#33](https://github.com/Dach-Coin/rlm-tools-bsl/issues/33)). Удалить конфиг вместе со службой можно флагом `--purge`.
...
### `rlm_start` падает с `PermissionError [WinError 5]`

**Симптом.** Первый же `rlm_start` возвращает ошибку вида:

```json
{"error": "Session init failed: PermissionError: [WinError 5] Отказано в доступе"}
```

**Причина.** По умолчанию (`RLM_SANDBOX_MODE=process`) код агента выполняется в
отдельном процессе на каждую сессию, и до его запуска сервер создает объекты ядра:
именованный канал (`Pipe`), разделяемую память (`RawArray`/`RawValue`) и два
семафора (`Lock`). Окружение с файловой песочницей — типично для некоторых
MCP-клиентов — может запретить дочернему процессу любой из них. Сессия тогда не
поднимается вовсе, и до сборки индекса дело даже не доходит: `rlm_start` индекс
не собирает. С версии 1.35.2 к этой ошибке добавляется подсказка с обходным путем.

**Лечение.** Задайте в секции `env` записи MCP-сервера:

```json
"RLM_SANDBOX_MODE": "inline"
```

**Честная цена.** Inline — это прежнее выполнение в основном MCP-процессе:

- процессной изоляции нет: namespace и память сессий общие, падение кода агента
  задевает сам сервер;
- таймаут перестает быть hard-kill: используется мягкий fallback (`SIGALRM` на
  Unix, асинхронное исключение потока на Windows), который код может пережить —
  зависший C-extension или блокирующий ввод-вывод им не снимаются;
- лимит памяти на сессию не действует.

Автоматического отката `process` -> `inline` в продукте нет и не будет: опечатка
или временный сбой не имеют права незаметно отключить заявленную границу
изоляции. Переключение — только явное решение оператора.
