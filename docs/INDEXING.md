...
**История коммитов (v1.34.0).** Тот же opt-in включает `git_log` / `git_commit` / `git_pickaxe`: кто и когда менял объект, состав одного коммита (без полного патча), в каком коммите появилась или исчезла строка. Это не индекс SQLite и не `git_search` (снимок «сейчас»). Пустой `path` ограничен корнем конфигурации, не всем git-репозиторием. Таймаут — `RLM_GIT_HISTORY_TIMEOUT`.
