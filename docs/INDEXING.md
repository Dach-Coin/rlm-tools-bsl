...
| Целевой язык                      | 10+ языков             | 10+ языков                     | любой                | заточен под BSL/1С (regex-грамматика, форматы CF/EDT/v8unpack, XML-метаданные CF/EDT)                     |
...
Формат исходников определяется автоматически. Для `v8unpack` нужен корневой
`Configuration.json` с непустым строковым полем `v8unpack` и поддерживаемое
BSL-дерево. JSON-метаданные объектов пока не разбираются. После обновления с
версии индекса ниже 15 выполните `index update`: команда автоматически сделает
полную перестройку структурных координат.
